...
function createMcpServer(octokitClient: Octokit, renderToken: string | undefined) {
  ...
  server.registerTool('workspace_push', {
    ...
    inputSchema: {
      owner: z.string().describe('Repository owner'),
      repo: z.string().describe('Repository name'),
      branch: z.string().describe('Target repository branch'),
      path: z.string().describe('Remote file path in the repository to update'),
      localPath: z.string().describe('Local path on the sandbox disk where the modified file is located'),
      message: z.string().describe('Commit message for the push')
    }
  }, async ({ owner, repo, branch, path: remotePath, localPath, message }) => {
    try {
      // Read the file natively from local sandbox workspace disk
      const fileBuffer = await fs.readFile(localPath);
      const base64Content = fileBuffer.toString('base64');

      // Attempt to retrieve any existing tracking blob SHA from GitHub to update correctly
      ...
      try {
        ...
        if (fileRes.data && !Array.isArray(fileRes.data) && 'sha' in fileRes.data) {
          existingSha = fileRes.data.sha;
        }
      } catch (err: any) {
        // If file is not found (404), continue without tracking SHA (new file creation)
        if (err.status !== 404) {
          throw err;
        }
      }

      // Execute commit over Octokit
      const res = await octokitClient.repos.createOrUpdateFileContents({
        owner,
        repo,
        path: remotePath,
      ...
      return formatSuccess(`Workspace push successful. Committed directly to branch ${branch}. Commit SHA: ${res.data.commit.sha}`);
    } catch (err) {
      return handleGitHubError(err);
    }
  });
  ...
  server.registerTool('workspace_pull', {
    ...
    inputSchema: {
      owner: z.string().describe('Repository owner'),
      repo: z.string().describe('Repository name'),
      branch: z.string().describe('Remote repository branch'),
      path: z.string().describe('Remote file path in the repository to fetch'),
      localPath: z.string().describe('Destination local path on the sandbox workspace disk where the file will be saved')
    }
  }, async ({ owner, repo, branch, path: remotePath, localPath }) => {
    try {
      // Fetch the raw file details and content from the remote repository
      const fileRes = await octokitClient.repos.getContent({
        owner,
        repo,
        path: remotePath,
      ...
      if (Array.isArray(fileRes.data) || !('content' in fileRes.data)) {
        throw new Error('Target remote path is not a file.');
      }

      const fileContentBase64 = fileRes.data.content;
      const fileBuffer = Buffer.from(fileContentBase64, 'base64');

      // Ensure local target directory structure is present
      const targetDir = path.dirname(localPath);
      await fs.mkdir(targetDir, { recursive: true });

      // Write raw binary buffer directly into the execution container disk
      await fs.writeFile(localPath, fileBuffer);

      return formatSuccess(`Workspace pull completed. File from ${remotePath} written directly to ${localPath} (${fileBuffer.length} bytes).`);
    } catch (err) {
      return handleGitHubError(err);
    }
  });
