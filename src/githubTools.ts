...
import { z } from 'zod';
import { formatOptimizedResponse, formatError, getToolAnnotations, resolveInputString, getSessionContext, updateSessionContext, makeRegistrar, sanitizeSessionEnv, validateRegex, safeRegexTest, RESOURCE_LIMITS } from './security.js';
...
export function registerGitHubTools(server: McpServer, octokit: Octokit, sessionId: string, registry: Record<string, any>) {
  ...
  reg('set_active_context', {
    ...
    if (env !== undefined) {
      try { patch.env = sanitizeSessionEnv(env); } catch (err: any) { return formatError(err); }
    }
  ...
  reg('str_replace_editor', {
    ...
    try {
      ...
      // Silently replacing only the FIRST match when old_str appears more than once is
      // a correctness hazard: the caller has no way to know which occurrence changed, or
      // that other identical blocks were left untouched. Require an unambiguous match.
      ...
      if (occurrences.length > 1) {
        ...
        const detail = occurrences.map((_, i) => `Occurrence ${i + 1}: starts at line ${lines[i]}`).join('\n');
        throw new Error(`old_str is ambiguous; found ${occurrences.length} occurrences. No changes were made.\n\n${detail}\n\nProvide a more specific old_str so exactly one occurrence matches.`);
      }
  ...
  reg('list_issue_types', {
    ...
    try {
      ...
      const res = await octokit.request('GET /repos/{owner}/{repo}/issue-types', { owner: target.owner, repo: target.repo });
      return formatOptimizedResponse(res.data);
    } catch (err) { return handleGitHubError(err); }
  });
  ...
  reg('search_repositories', {
    ...
    try {
      ...
      if (regex) {
        try { validateRegex(regex, 'i'); } catch (err: any) { return formatError(err); }
      }
  ...
  reg('add_comment_to_pending_review', {
    ...
    try {
      ...
      const res = await octokit.pulls.createReview({
        owner: target.owner, repo: target.repo, pull_number, commit_id: activeCommitId,
        event: '' as any, comments: [{ path, line, side, body }]
      });
  ...
  reg('push_files', {
    ...
    inputSchema: {
      ...
      branch: z.string(),
      message: z.string(),
      files: z.array(z.object({ path: z.string().min(1), content: z.string() })).min(1).max(RESOURCE_LIMITS.maxPushFiles)
    },
    ...
    try {
      ...
      // Validate paths and enforce size caps up front — before making any API calls —
      // so a bad request fails fast instead of partway through a multi-file commit.
      const unique = new Set<string>();
      let totalBytes = 0;
      for (const f of files) {
        const normalized = f.path.replace(/^\/+/, '');
        if (!normalized || normalized.split('/').some((p: string) => p === '..' || p === '.') || normalized.includes('\0')) {
        ...
        if (bytes > RESOURCE_LIMITS.maxPushFileBytes) throw new Error(`File '${f.path}' exceeds ${RESOURCE_LIMITS.maxPushFileBytes} bytes.`);
        totalBytes += bytes;
      }
      if (totalBytes > RESOURCE_LIMITS.maxPushTotalBytes) throw new Error(`Total push size exceeds ${RESOURCE_LIMITS.maxPushTotalBytes} bytes.`);

      // Single atomic commit via the Git Data API, instead of one createOrUpdateFileContents
      // call per file: a partial failure previously left the branch with some files
      // updated and others not, with no way to tell which had landed.
      ...
      const entries = (baseTree.data.tree || [])
        .filter((e: any) => e.path && e.type === 'blob')
      ...
      for (const f of files) {
        ...
        const idx = entries.findIndex((e: any) => e.path === normalized);
        const entry = { path: normalized, mode: '100644' as const, type: 'blob' as const, sha: blob.data.sha };
        if (idx >= 0) entries[idx] = entry; else entries.push(entry);
      }

      const tree = await octokit.git.createTree({ owner: target.owner, repo: target.repo, base_tree: baseCommit.data.tree.sha, tree: entries as any });
      const commit = await octokit.git.createCommit({ owner: target.owner, repo: target.repo, message, tree: tree.data.sha, parents: [baseSha] });
      await octokit.git.updateRef({ owner: target.owner, repo: target.repo, ref: `heads/${branch}`, sha: commit.data.sha, force: false });
  ...
  reg('sub_issue_write', {
    ...
    try {
      ...
      const attached = await octokit.request('POST /repos/{owner}/{repo}/issues/{issue_number}/sub_issues', {
        owner: target.owner, repo: target.repo, issue_number: parent_issue_number, sub_issue_id: created.data.id
      });
