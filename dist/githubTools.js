...
function resolveRepo(inputOwner, inputRepo, sessionId) {
...
export function registerGitHubTools(server, octokit, sessionId, registry) {
    ...
    reg('list_issue_types', {
        ...
        try {
            ...
            return formatOptimizedResponse(res.data.map((t) => t.name));
        }
        catch (err) {
            if (err?.status === 404)
                return formatOptimizedResponse(['Bug', 'Feature', 'Task', 'Improvement']);
            return handleGitHubError(err);
        }
    });
    reg('list_issues', {
