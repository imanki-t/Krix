...
export function updateSessionContext(sessionId, patch) {
...
export function makeRegistrar(server, registry) {
    return (name, config, handler) => {
        registry[name] = server.registerTool(name, config, handler);
        return registry[name];
    };
}
export function compressResponseData(data, maxChars = 2500) {
    ...
    if (typeof data === 'object') {
        ...
        for (const [key, value] of Object.entries(data)) {
            if (keysToOmit.has(key) || value === null || value === undefined || value === '')
                continue;
...
export function formatOptimizedResponse(data, maxChars) {
...
export function formatError(error) {
    ...
    return {
        isError: true,
        content: [{ type: 'text', text: `Error: ${sanitizeOutput(message)}` }]
    };
}
...
export function sanitizePath(inputPath, rootDir = process.cwd()) {
...
export function sanitizeCommand(cmd) {
    ...
    for (const pattern of dangerous) {
        if (pattern.test(cmd))
            throw new Error('Security Alert: Command blocked due to safety policy.');
    }
}
export function resolveInputString(plain, b64) {
    if (b64)
        return Buffer.from(b64, 'base64').toString('utf-8');
    return plain || '';
}
export function safeRegexTest(pattern, flags, text, timeoutMs = 200) {
    const context = { text, pattern, flags, result: false, error: null };
    try {
        const code = `
      try {
